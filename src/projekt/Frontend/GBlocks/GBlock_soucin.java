...
/**
 * Graficke zobrazeni bloku soucin
 * @author Michal Beranek(xberan38)
 */
public class GBlock_soucin extends GBlock{

    GPort port_out1, port_in1, port_in2;
    Label op;
    Block block;
     ...
     * @param id id bloku
     */
    public GBlock_soucin(double transx, double transy, int id){
        super(id);
        Main.id += 1;
        if(Main.plan1.add_block(Integer.toString(id), "soucin", hodnoty.Metr)) {
            block = Main.plan1.find_block(Integer.toString(id));
            op = new Label("*");
            port_out1 = new GPort("Metr^2", block.return_end_ports()[0]);
            ...
            GBlock_soucin.setAlignment(port_out1, Pos.CENTER_RIGHT);
            GBlock_soucin.setAlignment(port_in1, Pos.CENTER_LEFT);
            GBlock_soucin.setAlignment(port_in2, Pos.BOTTOM_LEFT);
            GBlock_soucin.setAlignment(op, Pos.TOP_CENTER);
        }
