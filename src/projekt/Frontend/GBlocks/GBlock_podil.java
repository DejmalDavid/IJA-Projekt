...
/**
 * Graficke zobrazeni bloku podil
 * @author Michal Beranek(xberan38)
 */
public class GBlock_podil extends GBlock{
    ...
    public GBlock_podil(double transx, double transy, int id){
        ...
        if(Main.plan1.add_block(Integer.toString(id), "podil", hodnoty.Metr)) {
            ...
            GBlock_podil.setAlignment(port_in1, Pos.CENTER_LEFT);
            GBlock_podil.setAlignment(port_in2, Pos.BOTTOM_LEFT);
            GBlock_podil.setAlignment(op, Pos.TOP_CENTER);
        }
